...
class Block {
    ...
    validateTransactions() {
        ...
        this.transactions.forEach(transaction => {
            // Here we would validate the signature of each transaction
            const isValid = SignatureUtil.verifyTransactionSignature(transaction);
            if (!isValid) {
    ...
    // Add mining reward transaction to the block
    addMiningReward(minerAddress) {
        const rewardTransaction = {
            id: 'reward-' + Date.now(),
            hash: CryptoUtil.hash('reward-' + Date.now()), // Simple reward transaction hash generation
            type: 'reward',
            data: {
                inputs: [],
                outputs: [{
                    address: minerAddress,
                    amount: Config.MINING_REWARD // Set mining reward
                }]
            }
        };
        this.transactions.push(rewardTransaction); // Add reward transaction to block
    }
}
