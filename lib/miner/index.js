        ...
        R.forEach((transaction) => {
            ...
            if (R.all(R.equals(false), transactionInputFoundAnywhere)) {
                if (transaction.type === 'regular' && negativeOutputsFound === 0) {
                     const isValid = CryptoEdDSAUtil.verifySignature(transaction.data.outputs[0].address, transaction.signature, transaction.toHash());
                    if (isValid) {
                        selectedTransactions.push(transaction);
                    } else {
                        rejectedTransactions.push(transaction);  // 签名无效，拒绝该交易
                    }
